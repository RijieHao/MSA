...
class MultimodalDataset(Dataset):
    def __init__(self, video_dir, csv_path, audio_dir=None):
        """
        初始化数据集，动态提取特征。
        """
        self.data = extract_features(video_dir, csv_path, audio_dir)

    def __len__(self):
        return len(self.data["class_labels"])

    def __getitem__(self, idx):
        """
        返回单个样本，包括文本、音频、视觉特征和标签。
        """
        return {
            "id": self.data["id"][idx],
...
def get_dataloaders(modalities=None, batch_size=BATCH_SIZE, num_workers=2, use_all_data=False, force_test_mode=False):
    ...
    return dataloaders
'''
def get_dataloaders(video_dir, csv_path, batch_size=32, audio_dir=None,all_test=False):
    """
    返回训练、验证和测试集的 DataLoader。
    """
    train_ratio = 0.8
    valid_ratio = 0.15

    dataset = MultimodalDataset(video_dir, csv_path, audio_dir=audio_dir)
    if all_test:
        # 全部数据作为测试集
