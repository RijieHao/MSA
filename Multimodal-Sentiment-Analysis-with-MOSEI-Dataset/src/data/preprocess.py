    ...
    def load_aligned_data(self):
        ...
        try:
            ...
            if not self.raw_data_dir.exists():
                ...
                logger.error("Please ensure .csd files are in the correct directory")
                return None
            
            # 修复：使用正确的文件路径
            dataset = {}
            dataset["language"] = str(self.raw_data_dir / "CMU_MOSEI_TimestampedWords.csd")
            dataset["acoustic"] = str(self.raw_data_dir / "CMU_MOSEI_COVAREP.csd") 
            dataset["visual"] = str(self.raw_data_dir / "CMU_MOSEI_VisualFacet42.csd")
            dataset["labels"] = str(self.raw_data_dir / "CMU_MOSEI_Labels.csd")

            # 检查所有文件是否存在
            missing_files = []
            for modality, file_path in dataset.items():
            ...
            for modality, file_path in dataset.items():
                logger.info(f"  {modality}: {file_path}")
